...
export function PetugasLoginCreator() {
  ...
  return (
    <div className="mb-5 rounded-3xl border border-blue-200 bg-blue-50/80 p-4 dark:border-blue-500/20 dark:bg-blue-500/10">
      <div className="mb-4">
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
        <label className="space-y-2">
          <span className="text-sm font-bold">Nama Petugas</span>
          <Input value={namaPetugas} onChange={(event) => setNamaPetugas(event.target.value)} />
        </label>
        <label className="space-y-2">
          <span className="text-sm font-bold">Jenis</span>
          <select className="h-11 w-full rounded-2xl border border-[var(--border)] bg-white/75 px-4 text-sm outline-none dark:bg-white/10" value={jenis} onChange={(event) => setJenis(event.target.value as "PCL" | "PML")}>
        ...
        <label className="space-y-2">
          <span className="text-sm font-bold">Password Awal</span>
